...
// pull from. status.conditions carries both answers as Claimed and
// Sourceable, and the operator deletes the mirror once either has read
// False for longer than the grace period.
//
// A claim is expressed in one of two ways, and never as a label.
// metadata.ownerReferences names each MxlReceiver that wants this flow
// on this target node, as a non-controller, non-blocking owner --
// consumers must not set controller=true, because multiple owners
// cannot all be controllers. spec.requestor names the single pod
// behind an on-demand materialization, and is what a cross-namespace
// mirror carries, since the apiserver rejects an owner reference that
// crosses namespaces. The current set of consumers is visible via
// kubectl describe.
